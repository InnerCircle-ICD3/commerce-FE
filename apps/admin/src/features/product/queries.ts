...
import { getProducts, getProductById, deleteProduct, updateProductStatus } from './api';
import type { Product, ProductListParams } from './api';
import type { PageResponse } from '@/shared/api/types';

// 상품 원본 타입에서 API 형식에 맞게 변환하는 유틸
...
export const productsQueryOptions = (params: ProductListParams) =>
  queryOptions({
    ...
    queryFn: async () => {
      const apiParams = {
        ...params,
        isSoldOut: params.sellingStatus === 'SOLD_OUT' ? true : 
                   params.sellingStatus === 'SELLING' ? false : undefined
      };
