...
// 메시지 아이템 컴포넌트 - 불필요한 리랜더링 방지를 위해 분리
  const MessageItem = memo(({ message: msg }: { message: ChatMessage }) => (
    <div 
      className={`mb-4 ${msg.is_admin ? 'text-right' : 'text-left'}`}
    >
      <div 
...
export default function ChatConversation({ room }: ChatConversationProps) {
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      ...
      .on('postgres_changes', {
        ...
        // 새 메시지와 동일한 ID의 메시지가 있는지 확인
        const hasDuplicate = messages.some(msg => msg.id === newMessage.id);
        if (!hasDuplicate) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // 메시지가 전송되었지만 실시간 구독으로 받지 못할 경우를 대비해 현재 메시지 표시
      const fallbackTimer = setTimeout(() => {
        // 0.5초 후에도 실시간 구독으로 받지 못했을 때 UI에 메시지 추가
        if (lastMessageRef.current === newMessageId) {
          console.log('[Admin ChatConversation] 실시간 구독으로 받지 못하여 직접 추가:', newMessageId);
          ...
          // requestAnimationFrame을 사용하여 렌더링 후 스크롤
          requestAnimationFrame(() => scrollToBottom());
        }
      }, 500);
      ...
      <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
        <div>
          <h2 className="font-bold text-gray-800">
            {hasProduct ? `${product.title} 문의` : '일반 문의'}
          </h2>
